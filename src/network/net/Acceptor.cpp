...
void Acceptor::Open()
{
    ...
    if(fd_ < 0)
    {
        NETWORK_LOG_ERROR<<"Acceptor Open failed. errno: "<<errno;
        exit(-1);
    }
    if(socket_opt_){
        delete socket_opt_;
    ...
    socket_opt_->SetReusePort(true);socket_opt_->BindAddress(addr_);
    socket_opt_->Listen();
...
void Acceptor::OnRead()
{
    ...
    while(true)
    {
        ...
        else
        {
            ...
            {
                NETWORK_LOG_ERROR << "Acceptor OnRead error errno=" << errno;
                OnClose();
            }
            break;
...
{
    NETWORK_LOG_ERROR << "Acceptor OnError: " << msg;
    OnClose();
}
