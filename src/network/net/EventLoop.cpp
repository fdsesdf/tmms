...
EventLoop::EventLoop():epoll_fd_(::epoll_create(1024)),epoll_events_(1024)
{
    ...
    {
        NETWORK_LOG_ERROR<<"there is already a event loop in this thread";
        exit(-1);
    }
    ...
    while(looping_)
    {
        ...
        if(ret>=0)
        {
            ...
            wheel_.OnTimer(now);
             ...
             NETWORK_LOG_ERROR<<"epoll wait error.error:"<<errno ;
        }
    }
...
bool EventLoop::EnableRead(const EventPtr & event, bool enable)
{
    auto iter = events_.find(event->Fd());
    if(iter == events_.end())
    {
        NETWORK_LOG_ERROR<<"event not found.fd:"<<event->Fd();
        return false;
    }
    if(enable)
    {
...
bool EventLoop::EnableWrite(const EventPtr & event, bool enable)
{
    auto iter = events_.find(event->Fd());
    if(iter == events_.end())
    {
...
void EventLoop::AssertInLoopThread()
{
    if(!IsInLoopThread())
    {
        NETWORK_LOG_ERROR<<"not in loop thread";
        exit(-1);
    }
